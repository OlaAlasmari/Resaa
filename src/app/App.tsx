...
// --- Types ---
type ViewState = 'login' | 'register' | 'home' | 'auction-browse' | 'auction-detail' | 'live-bidding' | 'my-auctions' | 'add-auction' | 'favorites' | 'profile' | 'bid-history' | 'support' | 'wallet' | 'faq';
type ParticipationRole = 'principal' | 'agent' | null;
type SellerRole = 'principal' | 'agent' | 'marketer';
      ...
      <div className="hidden md:flex flex-1 justify-start items-center gap-15 -translate-x-30">
        ...
        <button 
          ...
          className={`text-white/80 hover:text-red-500 transition-colors ${
            currentView === 'favorites' ? 'text-red-500' : ''
          }`}
          title="المفضلة"
        >
...
const AuthView = ({ isRegister, onToggle, onLogin }: { isRegister: boolean, onToggle: () => void, onLogin: () => void }) => (
...
export default function App() {
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-[#f8fafc] font-sans text-[#30364F] selection:bg-[#30364F] selection:text-white flex flex-col relative overflow-x-hidden">
       ...
       {currentView !== 'login' && currentView !== 'live-bidding' && (
         <Navbar 
            onNavigate={navigate} 
          ...
          <AnimatePresence mode="wait">
             ...
             {currentView === 'login' && (
                <AuthView 
                  ...
                  onLogin={() => { setIsLoggedIn(true); navigate('home'); }}
                />
             )}
